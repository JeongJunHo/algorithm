...
public class 토마토_2차원 {
	...
	public static void main(String[] args) throws IOException {
		...
		//상자 생성 n과 m 순서 주의 이번문제는 가로가 먼저 입력되었다.
		arr = new int[n][m];
		//박스에 토마토를 채운다.
		for (int i = 0; i < n; i++) {
		...
		while (!complete && !ripePos.isEmpty()) {
			...
			for (int i = 0; i < currentRipe; i++) {
				...
				if(!complete) {
					...
					//4방위 내의 안익은 토마토를 익게 만들고 큐에 삽입한다.
					for (int j = 0; j < pos.length; j++) {
						//상하좌우 반복하며 각각의 좌표 획득
						int nrow = ripeTomato[0] + pos[j][0];
						int ncol = ripeTomato[1] + pos[j][1];
						//상자안이면서 안익은 토마토라면
	...
	static boolean PossibleCheck() {
		...
		while (!greenPos.isEmpty()) {
			...
			for (int i = 0; i < pos.length; i++) {
				//새로운 좌표
				int nrow = greenTomato[0] + pos[i][0];
				int ncol = greenTomato[1] + pos[i][1];
				//해당 좌표가 범위밖이거나 빈칸일 경우
				if (!posCheck(nrow, ncol) || arr[nrow][ncol] == -1) {
					//막힌 횟수 카운트
					blocking++;
				}
			}
