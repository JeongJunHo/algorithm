package baekjoon.bj17141_연구소_2;
...
public class 연구소_2 {
	...
	//연구소 크기, 바이러스 수, 빈방의 수, 답
	static int n, m, room, ans = Integer.MAX_VALUE;
	//바이러스가 들어갈 수 있는 좌표 리스트
	...
	static void bfs() {
		...
		//바이러스를 놓을 수 있는 자리 리스트를 순회하며 현재 사용하기로 한 자리의 좌표를 큐에 삽입하고 방문한 곳으로 체크해준다.
		...
		while (!q.isEmpty()) {
			...
			for (int i = 0; i < size; i++) {
				...
				for (int j = 0; j < 4; j++) {
					...
					//범위안이면서 벽이 아니고(바이러스를 놓을 수 있는 자리 또는 빈방) 방문하지 않았다면
